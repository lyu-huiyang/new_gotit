# -*- coding: utf-8 -*-
from app import db


class User(db.Model):
    __tablename__ = 'users'
    id = db.Column(db.Integer, primary_key=True)
    stu_number = db.Column(db.String(64), unique=True)
    wechat_id = db.Column(db.String(64), unique=True, primary_key=True)
    zhengfang_password = db.Column(db.String(64), unique=True)
    library_password = db.Column(db.String(64), unique=True)

    def __init__(self, stu_number, wechat_id, zhengfang_password, library_password):
        self.stu_number = stu_number
        self.wechat_id = wechat_id
        self.zhengfang_password = zhengfang_password
        self.library_password = library_password

    def __repr__(self):
        return '<User %r>' % self.stu_number
